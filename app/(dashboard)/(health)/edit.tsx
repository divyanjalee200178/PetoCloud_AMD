...
const HealthEditScreen = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      router.back(); // go back to the list after update
    } catch (err) {
      console.error(err);
  ...
  return (
    <View className="flex-1 p-5">
      ...
      <TextInput
        ...
        className="border p-2 mb-2 rounded"
        value={record.day}
        onChangeText={text => setRecord({ ...record, day: text })}
      />
      <TextInput
        placeholder="Time"
        className="border p-2 mb-2 rounded"
        value={record.time}
        onChangeText={text => setRecord({ ...record, time: text })}
      />
      <TextInput
      ...
      <TextInput
        ...
        onChangeText={text => setRecord({ ...record, petName: text })}
      />
      <TextInput
        placeholder="Pet Age"
        keyboardType="numeric"
        className="border p-2 mb-2 rounded"
        value={record.petAge.toString()}
        onChangeText={text => setRecord({ ...record, petAge: Number(text) })}
      />
